...
interface BindSemEstoque {
    itemBarCode: string
    itemTitle: string
    QtEstoque_Empresa17: number | null
}

export default function BindSemEstoque() {
    const [isLoading, setIsLoading] = useState(true)
    const [isRefreshing, setIsRefreshing] = useState(false)
    const [data, setData] = useState<BindSemEstoque[]>([])
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch('/api/bind-sem-estoque', {
                method: 'GET',
                headers: {
    ...
    const handleExportToExcel = () => {
        try {
            ...
            const exportData = data.map(item => ({
                ...
                'Estoque': item.QtEstoque_Empresa17 ?? 0
            }))
            ...
            const fileName = `SOBRAL_bind_sem_estoque_${date}.xlsx`
